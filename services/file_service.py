...
from core.database import get_row, update_row, insert_history, insert_error
...
def enrich_file_destination(file_id: str) -> Dict[str,Any]:
    ...
    if not f: raise RuntimeError('Arquivo não encontrado')
    mapping = resolve_folder(f.get('project_detected',''), f.get('discipline_detected',''), f.get('autodoc_path',''), f.get('extension',''))
    scored = score_file(f, mapping)
    update = {**scored}
    if mapping:
        site = mapping['_site']
        folder_path = target_path(mapping, settings.app_env)
        update.update({'sharepoint_site_ref': site['id'], 'sharepoint_drive_id': site['drive_id'], 'sharepoint_suggested_path': folder_path, 'sharepoint_folder_map_ref': mapping.get('id')})
    return update_row('autodoc_files', file_id, update)
...
def upload_hml(file_id: str) -> Dict[str,Any]:
    ...
    if settings.enable_obsolete_rule:
        ...
        if moved:
            insert_history(email_id=f.get('email_id'), file_id=file_id, action=st.ACTION_MOVIDO_OBSOLETO, file_name=file_name, from_path=f'{target_folder}/{file_name}', to_path=moved.get('webUrl'), environment='HML', status='OK')
    item = sharepoint.upload_small(drive_id, target_folder, file_name, local_path)
    updated = update_row('autodoc_files', file_id, {'status': st.FILE_SALVO_HML, 'sharepoint_item_id': item.get('id'), 'sharepoint_web_url': item.get('webUrl'), 'error_message': None})
    insert_history(email_id=f.get('email_id'), file_id=file_id, action=st.ACTION_UPLOAD_HML, file_name=file_name, to_path=item.get('webUrl') or f'{target_folder}/{file_name}', environment='HML', status='OK')
