...
def _clean_dict(data: Dict[str, Any]) -> Dict[str, Any]:
    """Remove chaves com None para evitar erro em colunas/valores opcionais."""
...
def _find_existing_file(email_id: str, file_name: str, autodoc_path_safe: str) -> Optional[Dict[str, Any]]:
    rows = list_rows(
        'autodoc_files',
        limit=1,
        filters={
...
def _upsert_file_by_identity(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Garante que o parser sempre retorne file_id real.
    """
    email_id = row.get('email_id')
    file_name = row.get('file_name')
    autodoc_path_safe = row.get('autodoc_path_safe') or make_autodoc_path_safe(row.get('autodoc_path', ''))

    row['autodoc_path_safe'] = autodoc_path_safe
    row['source_hash'] = row.get('source_hash') or _file_identity(email_id, file_name, autodoc_path_safe)
...
def _status_for_file(fd: Dict[str, Any], mapping: Optional[Dict[str, Any]], score: Dict[str, Any]) -> str:
...
def sp_discover(payload: SiteDiscover, _: bool = Depends(require_api_key)):
    try:
        return discover_site_from_url(payload.project_name, payload.site_url, payload.aliases, payload.library_name)
    except Exception as e:
        raise HTTPException(500, str(e))
...
def ingest(payload: EmailIngest, _: bool = Depends(require_api_key)):
    try:
        ...
        email = upsert_row('autodoc_emails', email_data, on_conflict='message_id')
        ...
        insert_history(
            ...
            payload={'message_id': payload.message_id, 'source': payload.source},
        )
...
def parse_saved_email(email_id: str, force: bool = False, _: bool = Depends(require_api_key)):
    ...
    try:
        update_row('autodoc_emails', email_id, {'processing_started_at': now_iso(), 'error_message': None})
        ...
        for fd in parsed.get('files', []):
            autodoc_path_safe = make_autodoc_path_safe(fd.get('autodoc_path', ''))
            ...
            status = _status_for_file({**fd, 'project_detected': project_detected}, mapping, scored)
            ...
            row = {
                ...
                'autodoc_path': fd.get('autodoc_path'),
                'autodoc_path_safe': autodoc_path_safe,
            ...
            f = _upsert_file_by_identity(row)
            created.append({
                'file_id': f.get('id'),
            ...
            insert_history(
                ...
                payload={'source': fd.get('source'), 'autodoc_path_safe': autodoc_path_safe},
            )

        update_row('autodoc_emails', email_id, {
        ...
        insert_error('PARSER_ERROR', str(e), email_id=email_id, payload={'email_id': email_id})
        update_row('autodoc_emails', email_id, {
...
def parse_latest(_: bool = Depends(require_api_key)):
    ...
    if not emails:
        return {'ok': False, 'message': 'Nenhum e-mail recebido pendente'}
...
def files_ready(limit: int = 100):
    return list_rows('autodoc_files', limit=limit, filters={'status': st.FILE_PRONTO})
...
def file_approve(file_id: str, _: bool = Depends(require_api_key)):
    f = update_row('autodoc_files', file_id, {'status': st.FILE_PRONTO, 'approved': True, 'approved_at': now_iso()})
...
def file_ignore(file_id: str, _: bool = Depends(require_api_key)):
    f = update_row('autodoc_files', file_id, {'status': st.FILE_IGNORADO, 'ignored_at': now_iso()})
...
def upload_local(file_id: str, file: UploadFile = File(...), _: bool = Depends(require_api_key)):
    ...
    return update_row('autodoc_files', file_id, {'local_path': str(dest), 'status': st.FILE_BAIXADO, 'download_finished_at': now_iso()})
...
def queue_robot(file_id: str, _: bool = Depends(require_api_key)):
    ...
    update_row('autodoc_files', file_id, {'status': st.FILE_AGUARDANDO_DOWNLOAD, 'needs_autodoc_robot': True, 'robot_queue_id': job.get('id')})
...
def autodoc_download_endpoint(payload: Dict[str, Any], _: bool = Depends(require_api_key)):
    ...
    try:
        ...
        if f:
            update_row('autodoc_files', file_id, {'local_path': res['local_path'], 'status': st.FILE_BAIXADO, 'download_finished_at': now_iso()})
        return res
    except Exception as e:
        if f:
